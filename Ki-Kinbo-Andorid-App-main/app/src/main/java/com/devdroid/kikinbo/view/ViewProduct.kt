...
//    lateinit var productDisplayHelper: ProductDisplayHelper
//
...
import com.devdroid.kikinbo.model.repository.ProductRepository
import android.os.Bundle
import android.widget.TextView
import android.widget.Toast
import androidx.activity.enableEdgeToEdge
...
import com.devdroid.kikinbo.viewmodel.ViewProductViewModel
import com.devdroid.kikinbo.viewmodel.ViewProductViewModelFactory
...
class ViewProduct : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val repository: IProductRepository = ProductRepository(databaseRef) // Use the interface type

        // Use ViewModelProvider with a custom factory
        viewModel = ViewModelProvider(this, ViewProductViewModelFactory(repository))
            .get(ViewProductViewModel::class.java)
        ...
        // Get product ID from intent and fetch data
        val productIdToFetch = intent.getStringExtra("productId") ?: ""
        if (productIdToFetch.isNotEmpty()) {
            fetchAndDisplayProductData(productIdToFetch)
        } else {
            Toast.makeText(this, "Product ID not found", Toast.LENGTH_SHORT).show()
        }
    }

    private fun fetchAndDisplayProductData(productId: String) {
        viewModel.getProductData(productId) { productViewData ->
            if (productViewData != null) {
                displayProductDetails(productViewData)
            } else {
                Toast.makeText(this, "Product not found", Toast.LENGTH_SHORT).show()
            }
        }
    ...
    private fun displayProductDetails(product: ProductDataModel) {
        productNameId.text = product.productName
        productPriceId.text = product.productPrice.toString() // Assuming productPrice is a Double, convert it to String
        productRatingId.text = product.productRating.toString() // Assuming productRating is a Double, convert it to String
        productCategoryId.text = product.productCategory
        productDetailsId.text = product.productDetail
    }
}
